...
Settings has an optional **Follow macOS desktop** switch, off by default. When it is on and you move to another Mission Control desktop, today's journal window selects the Doing stream with the same number and zooms it: Desktop 1 shows `YYYY-MM-DD.jm.md`, Desktop 7 shows `YYYY-MM-DD_7.jm.md`, and numbers above 9 work even though the Command-number shortcuts stop at 9. Journalist Mode stays in the background and never takes the keyboard from the app you switched to. If today's window or the matching stream does not exist, nothing changes. Only today's window follows, so a window still open after midnight stops following until you open the new day. Desktop numbers are read as Mission Control shows them, so keep "Automatically rearrange Spaces based on most recent use" turned off in System Settings, or the numbers will move as you work. The feature uses a private macOS interface; when that interface is unavailable, Settings says so, the switch is disabled, and turning it on reports an error instead of silently doing nothing.
