...
A desktop change is an actual Space switch on one display to a numbered Space other than the last numbered Space that display showed. Identity is compared by Space id, not by desktop number, so reordering or removing desktops (which renumbers them without a switch) never swallows the next switch. Leaving for a full-screen Space and returning to the same desktop is not a change, and a manual unzoom survives it.
...
A bound method `GetFollowDesktopStatus() {available, reason}` reports whether the monitor started and, when it did, whether the last observation failed; a later good snapshot reports recovery and clears it. The Settings switch stays operable for turning the setting off even when the monitor is unavailable, so a persisted enabled value can always be reverted. The Settings window reads it on open, disables the switch and shows the reason when it is unavailable. A day window whose settings have the feature on reads it at start-up and shows the reason in the error banner. (A `menu:error` sent from `ApplicationDidFinishLaunching` would be dropped: the webview registers its listeners after the runtime flushes queued scripts.)
...
2. Ignores the event when the setting is known to be off; the setting is mirrored in a ref that `settings:changed` updates synchronously so an event already in flight cannot apply through a stale render closure. While the setting is not yet known or the window is still opening today's day, the target is kept as pending and applied once the day shows.
...
A second adversarial review of the implemented branch produced nine findings. Fixed: same-desktop suppression now compares Space identity (a reorder or removal no longer swallows the next switch); the payload is swapped out under the follower mutex so a baseline wins over a queued snapshot; a failed baseline or observation is exposed through `GetFollowDesktopStatus` and cleared on recovery; the Objective-C snapshot autoreleases its string and runs in an autorelease pool because the package builds without ARC; the Settings switch can always be turned off; changes arriving while today's window is opening are kept; the setting is read from a synchronously updated ref. Left as is: the Settings save's unhandled-rejection pattern predates this work and the banner already shows the message.
